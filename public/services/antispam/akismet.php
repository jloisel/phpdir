<?php 
/**
 * Akismet Antispam service modified.
 * 
 ...
 */
class Akismet extends AbstractAntispamService {
	...
	/**
	 * Returns the description of the akismet service.
	 *
	 * @return string
	 */
	public function getDescription() {
		return 'Blocks comment spam.';
	}
	
	/**
	 * Returns the version of the Akismet service.
	 *
	 * @return string
	 */
	...
	public function getConfigurationForm() {
		return new AkismetConfigurationForm();
	}
	
	public function onValidConfigurationForm(sfI18nForm $form) {
		ServiceSetting::setValue(self::AKISMET_API_KEY,$form->getValue(self::AKISMET_API_KEY));
		ServiceSetting::saveSettings();
	}
	
	public function isSpam($type, $email, $site, $ip, $content, Website $website) {
		$this->setCommentAuthorEmail($email);
		$this->setCommentAuthorURL($site);
		$this->setCommentContent($content);
		$this->setUserIP($ip);
		$this->setCommentType($type);
		return $this->isCommentSpam();
	}
...
/**
 * Akismet configuration form.
 *
 * @author Jerome Loisel
 */
class AkismetConfigurationForm extends ServiceConfigurationForm {
	...
	public function configure() {
		$this->setWidgets(array(
			Akismet::AKISMET_API_KEY => new sfWidgetFormInput()
		));
		
		$this->setDefaults(array(
			Akismet::AKISMET_API_KEY => ServiceSetting::getValue(Akismet::AKISMET_API_KEY)
		));
		
		$this->widgetSchema->setHelps(array(
			Akismet::AKISMET_API_KEY => 'Get your API key at <a href="http://wordpress.com/api-keys/" onclick="window.open(this.href);return false;">www.wordpress.com</a>'
		));
		
		$this->setValidators(array(
			Akismet::AKISMET_API_KEY => new AkismetApiKeyValidator()
		));
...
/**
 * Akismet API Key validator.
 *
 * @author Jerome Loisel
 */
class AkismetApiKeyValidator extends sfValidatorSchema {
	
	protected function doClean($values){
		$akismet = new Akismet(Config::get('site_url'),$values);
		if(!$akismet->isKeyValid()) {
